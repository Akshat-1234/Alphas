...
def prettify_result(
    ...
    is_stats_df = is_stats_df.sort_values("sharpe", ascending=False)
...
def save_is_tests(is_tests: pd.DataFrame, alpha_id: str, region: str) -> None:
    """
    Save an alpha's check results (submission checks, and/or self-correlation and
    production-correlation checks) to a CSV file in the 'alphas_is_tests' folder.

    Args:
        is_tests (pandas.DataFrame): The DataFrame containing check results, as
            returned in get_specified_alpha_stats's "is_tests" field.
        alpha_id (str): The ID of the alpha.
        region (str): The region for which the checks were generated.
    """

    folder_path = "alphas_is_tests/"
    file_path = os.path.join(folder_path, f"{alpha_id}_{region}.csv")
    os.makedirs(folder_path, exist_ok=True)

    is_tests.to_csv(file_path, index=False)
